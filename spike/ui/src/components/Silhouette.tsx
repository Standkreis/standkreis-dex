...
// Plain shapes, viewBox 0 0 100 100. No AI imagery; these are hand-placed primitives.
...
export function Silhouette({ group, mode, className = '', bg = 'var(--color-tile)' }: { group: Group; mode: 'fill' | 'outline'; className?: string; bg?: string }) {
  const shapes = SHAPES[group]
  return (
    <svg viewBox="-6 -6 112 112" className={className} aria-hidden>
      {mode === 'fill' ? (
        <g fill="currentColor" dangerouslySetInnerHTML={{ __html: shapes }} />
      ) : (
        <>
          <g fill="currentColor" stroke="currentColor" strokeWidth="5" strokeLinejoin="round" dangerouslySetInnerHTML={{ __html: shapes }} />
          <g fill={bg} dangerouslySetInnerHTML={{ __html: shapes }} />
        </>
      )}
    </svg>
  )
...
export const silhouetteMask = (group: Group) =>
  `url("data:image/svg+xml,${encodeURIComponent(`<svg xmlns="http://www.w3.org/2000/svg" viewBox="-6 -6 112 112"><g fill="black">${SHAPES[group]}</g></svg>`)}")`
