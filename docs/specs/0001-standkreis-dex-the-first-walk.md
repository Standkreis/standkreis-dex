...
Bottom app bar **Dex · Quests · ＋ · Tagebuch · Du**; the ＋ is an action, not a tab. Home is **the dex under the global filter**, grid view in the first slice, list and map views deferred. Filters (region, tiles) are first-class and set during onboarding; the month is the default sort and the opt-in chip "nur jetzt". In slice one the Quests tab holds a single "kommt bald" line (Q10 stands); Tagebuch is the sightings by day; Du is counters, XP, groups, export, delete and the passkey upgrade, with Einstellungen behind a gear.
