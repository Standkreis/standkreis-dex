...
| Schema beyond the ER diagram | `Sighting.wildness` (wild · captive · cultivated), `Sighting.place`, `Asset.kind` (image · sound), `Asset.origin`, `Asset.owner`, `Taxon.facts`. ~~`Plausibility.source`, `Filter.wholeYear`~~ | Spec §⚖️ and §🎨 need them; the spike rendered them. All optional or defaulted, so the ETL can fill in stages. **Amended 2026-09-05 ([handoff 0006](0006-etl-and-identity.md) step 0):** plausibility is per `Region` and whole year with twelve month shares, `source` dropped (GBIF alone), `Group` became `Tile` with 🐟, `Filter` holds `regionId` · `tiles` · `nowOnly`, `Identity.credential` became `Passkey` rows plus a reserved `email`, `Lookalike` added, `Taxon` gained `class` · `order` · `genus` · `namePath` · `contentAt` |
