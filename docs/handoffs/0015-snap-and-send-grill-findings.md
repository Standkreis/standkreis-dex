...
| # | Question | Agent's proposal | Evidence | Decision |
| --- | --- | --- | --- | --- |
| B1 | Beats or matches | **Matches Claude set on hits, beats every engine on confident errors, loses on words.** C: ✅6 🟡4 🔸7 ❌1 vs Claude set ✅7 🟡5 ⬜3 ❌3, Pl@ntNet ✅1 🟡6 🔸9 ❌2. Mantis 1.00 | §B1 table | ✅ noted; loses on words, which is the product |
| B2 | Open set or the 929 | **C, never A as the answer, never B alone.** A for the "what is it then" behind "outside the set" | Bonsai → blackbird, *Schefflera* → dormouse in B; C repairs 2 of 3 and halves B's wrong confidences | ✅ C if ever built; distractor list per region is part of the cost |
| B3 | Misty trees | Same verdict as I2: fruit carries, silhouette does not. C: 2 ✅, 3 🟡, 5 🔸, **0 ❌** on the ten | §B3 | ✅ same verdict as I2 |
| B4 | Where | **Hetzner CX23, 5.99 €/month**, if the privacy sentence is worth a second host. Otherwise I8 "skip" stands | §B4 | ✅ owner 2026-09-06: **Hetzner CX23 is the fallback when Sonnet gets too expensive.** Not built now |
| B5 | Speed | 33 ms MPS · 132 ms CPU (2 threads) · 3 s load | §B5 | ✅ noted |
