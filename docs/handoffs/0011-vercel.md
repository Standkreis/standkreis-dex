...
| Store | `@vercel/blob`, the owner's store is **private** (`standkreis-dex-blob`, `iad1`): `put('photos/<assetId>.jpg', bytes, { access: 'private', addRandomSuffix: false, contentType: 'image/jpeg' })`; `del()` on remove; the blob's path is derived from the `Asset` id, nothing new is stored in the DB | Schema frozen; a private blob has no public URL at all, so the capability URL stays `/api/photo/<id>` and nothing else exists |
...
| Serving | `GET /api/photo/<id>` stays the URL the worker caches. Blob backend: the route fetches the private blob with the token (`get()` / the SDK's private read) and **streams** it with `Cache-Control: private, max-age=31536000, immutable`; disk backend unchanged | A private blob cannot be redirected to; the worker caches the streamed response under the request URL, so offline photos keep working. Cost: one function read per first view per device, then the worker's cache |
...
| Sweep | `GET /api/cron/sweep`: `Authorization: Bearer <CRON_SECRET>` (Vercel sends it), runs `sweep()`, returns the `SweepResult`; `vercel.json` `crons: [{ path: '/api/cron/sweep', schedule: '0 * * * *' }]` | Hourly on Pro; "heal what died", since the jobs themselves now finish in-request |
...
- **Blob region.** The store sits in `iad1` (US); a photo of a salamander is not personal data, the location is not in the file (EXIF stripped, M6). Proposal: accept; if EU residency ever matters, the seam takes Cloudflare R2 in an afternoon.
- **Cron cadence.** The team is on Pro, so the sweep may run hourly (`0 * * * *`). Proposal: hourly; it is idle work when nothing died.
