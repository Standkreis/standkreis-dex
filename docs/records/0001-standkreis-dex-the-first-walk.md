...
**2026-09-04 · Q8c, XP as private progression.** 🙋 The owner reaffirmed twice, against the agent's Q8 objection, that the profile carries XP and a level. Reading of the evidence: Di Cecco, Atsumi and the iNaturalist staff quotes are about *competitive, public, volume-rewarding* systems; none covers a private, capped tariff that pays for dex state changes. Q8c therefore refines Q8 rather than reversing its reasoning: **XP exists, private only.** Tests that keep it so, all product rules now: never on a share card, never compared to anyone, no decay, no streak, no rarity multiplier, repeat finds pay once per species and month, studied pays only after the two-question recap (until the recap exists, studied earns nothing). Level is a number, no names. The "no leaderboard" refusal stands unchanged; the copy "ohne Punkte" became "ohne Rangliste". Revisit if repeat-find gaming shows in real data. Evidence and mock: [findings §10](../handoffs/0002-ui-exploration-findings.md), [review 0003](../handoffs/0003-ui-spike-review.md).

**2026-09-04 · Q8, the freeze is gone.** 🙋 "Weekly with a freeze" collapses into the weekly cadence: with no streak, no XP on quests beyond the per-quest amount, and no weekly total, there is nothing a pause protects and three ignored suggestions cost nothing. The anti-binge intent of Q8 is carried by the weekly rhythm alone. If "quiet" is ever needed it is a notification mute, not a quest state.
