...
export function FilterDrawer(p: Props) {
  ...
  return (
    <Sheet onClose={p.onClose} labelledBy="filter-title" z="z-40" testId="drawer" handleTestId="drawer-handle" handleClassName="pb-1"
      handle={
          ...
          {p.tiles.map(({ tile, count }) => {
            ...
            return (
              <Chip key={tile} on={on} onClick={() => p.onToggleTile(tile)} role="checkbox" checked={on} testId={`tile-${tile}`}>
                ...
                {tt(tile)}<span className={`motion-toggle ml-1 text-[13px] font-normal ${on ? 'text-sky-deep/70' : 'text-ink-faint'}`}>{count}</span>
              </Chip>
            )
          })}
        </Section>
