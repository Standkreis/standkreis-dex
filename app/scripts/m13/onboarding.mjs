...
const clickRegion = () => evaluate(`(() => { const b = [...document.querySelectorAll('[data-testid=regions] button')].find((b) => /Mainz/.test(b.textContent)) ?? document.querySelector('[data-testid=regions] button'); b?.click(); document.querySelector('[data-testid=region-next]')?.click(); return b?.textContent ?? null })()`)
